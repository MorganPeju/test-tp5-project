...
def update_titles(my_df):
    ...
    my_df['properties.TYPE_SITE_INTERVENTION'] = my_df['properties.TYPE_SITE_INTERVENTION'].apply(lambda x: TITLES.get(x))
    return my_df
...
def sort_df(my_df):
    ...
    '''
    # TODO : Sort the df
    return my_df.sort_values('properties.TYPE_SITE_INTERVENTION').reset_index()
...
def get_neighborhoods(montreal_data):
    ...
    my_df = to_df(montreal_data)
    return my_df['properties.NOM'].to_numpy()
